...
def visualize(df_,path):
    ...
    ax_bar.set_xlabel("Liczba wspólnych filmów (nominacji)")
    ax_bar.set_title("Najczęściej współpracujące pary aktor–producent/reżyser")

    # tylko ticki całkowite na osi X
    ax_bar.xaxis.set_major_locator(MaxNLocator(integer=True))

    fig_bar.tight_layout()
    fig_bar.savefig(path / "actors_producers_top_pairs.png",
    ...
    sns.heatmap(
        ...
        linewidths=0.5,
        linecolor="#020617",
        cbar_kws={"label": "Liczba wspólnych filmów (nominacji)"}
    )

    ax_hm.set_xlabel("Producer / director")
    ax_hm.set_ylabel("Actor / actress")
    ax_hm.set_title("Współpraca aktorów z producentami/reżyserami (TOP 10 × TOP 10)")

    fig_hm.tight_layout()
    fig_hm.savefig(path / "actors_producers_heatmap.png",
                   dpi=300, bbox_inches="tight")
    plt.close(fig_hm)
    ...
    plt.title('Liczba nominowanych według kategorii i dekad', fontsize=16)
    plt.xlabel('Kategoria', fontsize=12)
    plt.ylabel('Dekada', fontsize=12)
    ...
    plt.xlabel('Dekada_płeć', fontsize=12)
    ...
    ax.set_title('Percentage share of nominations by category and decade',fontsize=16)
    ...
    ax.legend(handles,labels,title="Category",loc='center left',bbox_to_anchor=(1.02, 0.5),borderaxespad=0.,fontsize=6,title_fontsize=9, ncol=1,frameon=False)
    ...
    ax.set_title('Oskary, ilosc nominacje wg kategorii i dekady\nlewa połowa = kobiety, prawa połowa = mężczyźni')
    ...
    cbar_f.set_label('Liczba nominacji – kobiety')

    cbar_m = plt.colorbar(sm_m, ax=ax, fraction=0.03, pad=0.09)
    cbar_m.set_label('Liczba nominacji – mężczyźni')

    plt.tight_layout()
    plt.savefig(path / 'decades_categories_gender.png')
    plt.show()